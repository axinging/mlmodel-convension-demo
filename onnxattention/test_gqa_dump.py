...
def dumpGQAAsJson(name, suffix, op, inputs, outputs, attributes, opset):
    data = {}
    data["name"] = name
    data["operator"] = op
    data["opset"] = opset
    ...
    #    inputObj = ({"data": input, "dims": input.shape, "type": input.dtype},)
...
def parity_check_gqa_prompt_no_buff(
    ...
    print(" config.kv_num_heads " + str(config.kv_num_heads))
    print(" config.head_size " + str(config.head_size))
